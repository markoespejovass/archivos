...
**Fecha:** 2026-08-31 (re-verificación completa) · **Auditoría inicial:** 2026-08-20
...
**Documento vivo — versión 1.5.** Este informe se mantiene como base de trabajo para la remediación. El estado de cada hallazgo se registra en §20; el historial de cambios, al final del documento.
...
Se analizaron **11 microservicios Spring Boot independientes** (Java 17, arquitectura hexagonal, controladores generados con `openapi-generator` y `delegatePattern`), pertenecientes al programa **NexHub / Proyecto NUBE Peru** (`com.santander.cpe.nxhbsc`), con **682 archivos fuente** y **43 operaciones REST declaradas** en contratos OpenAPI (eran 67 en v1.4: los contratos se recortaron entre ambas revisiones, ver SEC-046).
...
* tres defectos **introducidos o revelados por los propios cambios** de este ciclo: dos servicios que asumen el rol IAM de otro servicio (SEC-054), el despliegue productivo de `becustombeprogrm` leyendo un secreto del almacén de **desarrollo** (SEC-055) y valores de relleno (`xx`, `test`, `12`) como defecto de variables de entorno en los perfiles `pre`/`pro` (SEC-056).

Se resuelven en este ciclo dos hallazgos: los **datos personales reales en `data.sql`** (SEC-008, fichero eliminado) y las **cabeceras del consumidor alimentando el motor de riesgo** (SEC-026, ahora constantes de servidor).
...
Se identificaron **55 hallazgos vivos**: 7 Critical, 18 High, 21 Medium, 6 Low y 3 Info. Respecto a la línea base del 24-ago (52 hallazgos), 2 están resueltos, 9 han mejorado (4 bajan de severidad y 5 reducen alcance), 2 empeoran y 5 son nuevos. No hay ninguna regresión. El detalle de la comparación está en §23.

**Lectura de la tendencia.** El equipo ha trabajado, y se nota: los secretos salieron mayoritariamente del árbol de fuentes, desaparecieron los datos personales de `data.sql`, se añadieron timeouts, apareció un rate limiter y los contratos se recortaron a lo realmente implementado. Pero **ninguno de los cuatro defectos estructurales se ha tocado**: la autenticación sigue desactivada en los once servicios, no hay autorización de objeto en ninguno, el callback de firma sigue apuntando a `webhook.site` y el OTP sigue sin vincularse al usuario. Y tres de los cinco hallazgos nuevos nacen de los propios cambios introducidos, lo que indica que las correcciones se están aplicando sin una verificación posterior que las valide.
...
| Severidad | v1.4 (24-ago) | **v1.5 (31-ago)** | Δ |
| --------- | ------------: | ----------------: | - |
| Critical  |             9 |             **7** | −2 |
| High      |            17 |            **18** | +1 |
| Medium    |            18 |            **21** | +3 |
...
**Movimientos entre las dos revisiones** (detalle completo en §23):

| Clase | N.º | Hallazgos |
| ----- | --: | --------- |
| **RESUELTO** | 2 | SEC-008, SEC-026 |
| **ATENUADO** (baja de severidad) | 4 | SEC-003 (Critical→High), SEC-009, SEC-016 y SEC-023 (High→Medium) |
| Alcance reducido, misma severidad | 5 | SEC-004 (7→6 proyectos), SEC-019 (1er rate limiter), SEC-043 (11→4), SEC-046 (67/26→43/38), SEC-052 (~26→5 endpoints) |
| Empeora sin subir de severidad | 2 | SEC-034 (la mezcla de Netty llega a `beclaims`), SEC-030 (remediación incompleta: todo `PATCH` válido da 500) |
| **PERSISTE** sin cambio | 39 | Incluidos los cuatro estructurales SEC-001, SEC-002, SEC-005 y SEC-006 |
| **NUEVO** | 5 | SEC-053, SEC-054, SEC-055, SEC-056, SEC-057 |

> Las tres clases centrales explican por qué `compare_scans.py` reporta 4 atenuados y 0 agravados mientras el texto habla de nueve mejoras y dos empeoramientos: el comparador clasifica por severidad, y siete de esos once movimientos cambian el alcance o el comportamiento sin cambiarla. El desglose está en §23.1.
...
| ID | Severidad | Confianza | Hallazgo | Proyecto | Componente | CWE | Prioridad | Δ v1.5 (31-ago) |
| -- | --------- | --------- | -------- | -------- | ---------- | --- | --------- | --------------- |
...
| SEC-005 | Critical | CONFIRMADO | Bypass de OTP: sin vínculo a usuario, sin límite, OTP en path y en logs | beemailboxes | `OTPServiceAdapter` | CWE-304, CWE-307 | P0 | — sin cambio · ver SEC-053 |
| SEC-006 | Critical | CONFIRMADO | Callback de firma digital a `webhook.site` y token de callback literal `jwt` | bedigitsignature | `Constant`, `DocumentProcessService` | CWE-200, CWE-306 | P0 | — sin cambio (`Constant:64-65`) |
...
| SEC-012 | High | CONFIRMADO | Tokens biométricos y datos de DNI escritos al log | beidentbiometric | `FacephiAdapter:84,124` | CWE-532 | P1 | — sin cambio (14 puntos de log) |
...
| SEC-014 | High | CONFIRMADO | `GET /documents/{id}/versions` ignora el id y devuelve un scan completo | bedocmanagement | `DocumentManagementAdapter:187-191` | CWE-200 | P1 | — sin cambio |
...
| SEC-016 | Medium | CONFIRMADO | Integraciones salientes sin timeout | 3 proyectos | `WebClientConfig`, `RestTemplateConfig` | CWE-1088 | P2 | **↓ High→Medium.** Queda solo `becustombeprogrm` |
| SEC-017 | High | CONFIRMADO | Reintentos sobre operaciones no idempotentes sin clave de idempotencia | becustombeprogrm | `SKYServiceAdapter:94` | CWE-837 | P1 | — sin cambio |
| SEC-018 | High | CONFIRMADO | Cuerpo de error del proveedor propagado al consumidor | becustombeprogrm, bedigitsignature | `SKYServiceAdapter`, `Util.handleError` | CWE-209 | P1 | — sin cambio |
...
| SEC-023 | Medium | CONFIRMADO | JWT M2M sin `iss`/`aud`/`jti`, clave AES reutilizada como clave JWT, expiración de 20 s | becustombeprogrm, bedigitsignature | `JwtUtil`, `SkyTokenService` | CWE-1270, CWE-613 | P2 | **↓ High→Medium.** Corregido en `becustombeprogrm`, no en `bedigitsignature` |
| SEC-024 | Medium | CONFIRMADO | AES sin cifrado autenticado y transformación tomada de configuración | becustombeprogrm | `AesEncryptionService:30-46` | CWE-353 | P2 | — sin cambio · la clave se sigue registrando en log (línea 33) |
...
| SEC-031 | Medium | CONFIRMADO | JWT almacenado como partition key en DynamoDB | bedigitsignature | `Entity.java:29-33` | CWE-522 | P2 | — sin cambio |
...
| SEC-034 | Medium | CONFIRMADO | Deriva de versiones; `bedatacomanagment` mezcla Netty 4.1.x y 4.2.x | Todos | `pom.xml` | CWE-1104 | P2 | **↑ Agravado.** La mezcla llega también a `beclaims` |
...
| SEC-039 | Medium | CONFIRMADO | `search_documents` ejecuta Scan completo de DynamoDB sin paginación | bedocmanagement | `BaseDynamoRepository:76-92` | CWE-405 | P2 | — sin cambio |
...
| SEC-043 | Low | CONFIRMADO | `automountServiceAccountToken: true` en despliegue productivo | Todos | `.gluon/cd/pro/values-pro.yaml` | CWE-250 | P3 | **Alcance 11→4.** Pendientes: beclaims, bedocmanagement, beknowyocustomer, beemailboxes |
...
| SEC-046 | Info | CONFIRMADO | 67 operaciones declaradas frente a ~26 implementadas | Todos | Contratos OpenAPI | — | P3 | **Atenuado.** Ahora 43 declaradas / 38 implementadas |
...
| SEC-048 | High | CONFIRMADO | El DNI del titular se devuelve en el campo `name` de los metadatos | bedocmanagement | `DocumentMapper:111-118` | CWE-359 | P1 | — sin cambio |
| SEC-049 | High | CONFIRMADO | `deleteDocument` sin verificación de titularidad ni de existencia | bedocmanagement | `DocumentManagementAdapter:199-216` | CWE-639 | P1 | — sin cambio |
| SEC-050 | Medium | CONFIRMADO | El path variable determina el verbo HTTP hacia el proveedor | becustombeprogrm | `QurableServiceAdapter:80-92,169-178` | CWE-470 | P2 | — sin cambio |
| SEC-051 | Medium | CONFIRMADO | Campos cruzados en `FileEntity`: `customerId` guarda el id del documento | bedocmanagement | `FileEntity:18-36`, `DocumentMapper:71-90` | CWE-1109 | P2 | — sin cambio |
| SEC-052 | Low | CONFIRMADO | Endpoints declarados que responden 501 Not Implemented | beknowyocustomer, bedatacomanagment | Contratos + delegates | CWE-1059 | P3 | **Atenuado.** De ~26 a 5; hoy en bedocmanagement (3), beknowyocustomer (1), beemailboxes (1) |
| SEC-053 | High | CONFIRMADO | La limitación de OTP cubre la generación pero no la validación y el registro crece sin cota | beemailboxes | `EmailboxIdInputPort:39-70`, `RateLimiterConfig:12-22` | CWE-307, CWE-770 | P1 | **NUEVO** |
| SEC-054 | High | CONFIRMADO | Dos servicios asumen el rol IAM de otro servicio | becustombeprogrm, bedigitsignature | `application-*.yml` role-arn | CWE-269, CWE-1268 | P1 | **NUEVO** |
| SEC-055 | High | CONFIRMADO | El despliegue productivo lee un secreto del almacén de desarrollo | becustombeprogrm | `.gluon/cd/pro/values-pro.yaml:37-42` | CWE-1188, CWE-798 | P1 | **NUEVO** |
| SEC-056 | Medium | CONFIRMADO | Valores de relleno como defecto de variables de entorno en perfiles pre y pro | 4 proyectos | `application-pro.yml`, `application.yml` | CWE-453, CWE-1188 | P2 | **NUEVO** |
| SEC-057 | Info | CONFIRMADO | El proyecto del Lambda Authorizer existe pero está vacío | lmauthorizer | `src/helloworld.js`, `deployment.yml` | — | P1 | **NUEVO** |
...
| Todos los endpoints? | Si, 43 operaciones declaradas en 11 contratos (eran 67 en v1.4) |
...
| ¿Cadenas de ataque? | Sí — 6 cadenas documentadas (§8) |
...
| ¿Se re-verificaron los hallazgos previos contra el código actual? | Sí — v1.5, los 52 de la línea base del 24-ago, uno a uno (§23) |
...
**Los 55 hallazgos vivos están todos en `ABIERTO`.** Aparte de ellos, **2 hallazgos de la línea base anterior se han cerrado** en este ciclo (SEC-008 y SEC-026) y salen del recuento; se registran en la tabla siguiente. Además, 4 hallazgos han bajado de severidad y 5 han reducido su alcance sin cambiar de severidad.

| Estado | Critical | High | Medium | Low | Info | Total |
| ------ | -------: | ---: | -----: | --: | ---: | ----: |
| ABIERTO | 7 | 18 | 21 | 6 | 3 | **55** |
| EN CURSO | 0 | 0 | 0 | 0 | 0 | 0 |
| EN REVISIÓN | 0 | 0 | 0 | 0 | 0 | 0 |
| CERRADO | 0 | 0 | 0 | 0 | 0 | 0 |
| ASUMIDO | 0 | 0 | 0 | 0 | 0 | 0 |

> **Por qué los dos cierres no figuran en esta tabla.** El recuento de 55 son los hallazgos vivos, y el Excel ejecutivo se genera de él; incluir ahí los cerrados haría que el porcentaje de avance del dashboard nunca partiera de cero. Los cierres tienen su propia tabla justo debajo.
>
...
| ID | Severidad original | Hallazgo | Proyecto | Verificación (2026-08-31) |
| -- | ------------------ | -------- | -------- | ------------------------- |
| SEC-008 | Critical | Datos personales reales (DNI + PLAFT/PEP) en `data.sql` versionado | becreditrisk | Fichero inexistente en todo el árbol; sin referencias en configuración. **Pendiente**: purga en BD e historial de Git |
| SEC-026 | Medium | Cabeceras `society`/`channel` del consumidor alimentan el motor de riesgo | becreditrisk | `CreditRiskMapper:41-42` fija `application` y `channel` como constantes; `society` eliminado del modelo |
...
| SEC-016 | Medium ↓ | P2 | becustombeprogrm | Integraciones salientes sin timeout | `ABIERTO` |  |  | 2026-08-31 | 2 de 3 corregidos. El que queda es el único que además reintenta |
...
| SEC-023 | Medium ↓ | P2 | becustombeprogrm, bedigitsignature | JWT M2M sin `iss`/`aud`/`jti`; 20 s en `bedigitsignature` | `ABIERTO` |  |  | 2026-08-31 | Corregido en un servicio y no en el otro |
...
| SEC-055 | High | P1 | becustombeprogrm | El chart de `pro` lee un secreto de `dev-publickey-nexhub` | `ABIERTO` |  |  | 2026-08-31 | Nuevo. Único cruce de entorno del conjunto |
...
| SEC-057 | Info | P1 | lmauthorizer | El proyecto del Lambda Authorizer existe pero está vacío | `ABIERTO` |  |  | 2026-08-31 | Nuevo. Confirmar con Arquitectura; condiciona la calibración de SEC-001 |
...
```text
security-baseline-2026-08-31-v1.5.json      ← vigente (55 hallazgos)
security-baseline-2026-08-24-v1.4.json      ← anterior (52 hallazgos)
...
python compare_scans.py security-baseline-2026-08-31-v1.5.json security-baseline-<fecha>.json --md comparativa.md
```
...
* **17 casos de prueba** concretos con la petición y el resultado esperado (CP-16 y CP-17 añadidos el 31-ago).
